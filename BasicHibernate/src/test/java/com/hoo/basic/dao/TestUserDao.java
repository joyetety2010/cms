...
import com.hoo.basic.test.util.AbstractDbUnitTestCase;
import com.hoo.basic.test.util.EntitiesHelper;
...
public class TestUserDao extends AbstractDbUnitTestCase{
	...
	public void setUp() throws DataSetException, SQLException, IOException {
		...
		System.out.println(s.getTransaction().isActive());
		this.backupAllTable();
	}
	...
	public void testListByArgs() throws DatabaseUnitException, SQLException {
		...
		List<User> expected = userDao.getUser("from User where id>? and id<?", new Object[]{1,4});
	...
	public void testListByArgsAndAlias() throws DatabaseUnitException, SQLException {
		...
		List<User> expected = userDao.getUser("from User where id>? and id<? and id in(:ids)", new Object[]{1,5},alias);
	...
	public void testFindByArgs() throws DatabaseUnitException, SQLException {
		...
		Pager<User> expected = userDao.findUser("from User where id>=? and id<=?", new Object[]{1,10});
	...
	public void testFindByArgsAndAlias() throws DatabaseUnitException, SQLException {
		...
		Pager<User> expected = userDao.findUser("from User where id>=? and id<=? and id in (:ids)", new Object[]{1,10},alias);
	...
	public void testListSQLByArgs() throws DatabaseUnitException, SQLException {
		...
		SystemContext.setOrder("desc");
		SystemContext.setSort("id");
		List<User> expected = userDao.listUserBySql("select * from t_user where id>? and id<?", new Object[]{1,4},User.class);
		List<User> actuals = Arrays.asList(new User(3,"admin3"),new User(2,"admin2"));
		assertNotNull(expected);
		assertTrue(expected.size()==2);
		EntitiesHelper.assertUsers(expected, actuals);
	...
	public void testListSQLByArgsAndAlias() throws DatabaseUnitException, SQLException {
		...
		Map<String,Object> alias = new HashMap<String,Object>();
		alias.put("ids", Arrays.asList(1,2,3,5,6,7,8,9,10));
		List<User> expected = userDao.listUserBySql("select * from t_user where id>? and id<? and id in(:ids)", new Object[]{1,5},alias,User.class);
		List<User> actuals = Arrays.asList(new User(2,"admin2"),new User(3,"admin3"));
		assertNotNull(expected);
		assertTrue(expected.size()==2);
		EntitiesHelper.assertUsers(expected, actuals);
